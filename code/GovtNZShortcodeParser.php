...
if (self::$debugging) Debug::show('tag is ' . print_r($orderedTag, true));
...
if (self::$debugging) Debug::show('node is ' . print_r($shortcodeNode, true));
if (self::$debugging) Debug::show('parent is ' . print_r($parent, true));
...
if (self::$debugging) Debug::show('end of loop content DOM is ' . $htmlvalue->getContent());
		}
...
if (self::$debugging) Debug::show('extract tags step 1:' . print_r($tags, true));
...
if (self::$debugging) Debug::show('replacing marker content with ' . print_r($content, true));	
		if ($content) {
...
if (self::$debugging) Debug::show('content from node is ' . print_r($content, true));
		...
		if ($content === false) {
			...
			else if(self::$error_behavior == self::STRIP) {
				return '';
			}
			else {
				return $tag['text'];
...
if (self::$debugging) Debug::show('after replacing: ' . print_r(substr($content, 0, $tags[0]['s']) . $str, true));
